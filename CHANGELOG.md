...
- **Behaviour-breaking:** The code generator now emits Pydantic field constraints (`min_length`, `max_length`, `ge`, `le`) that the model factory computes from FHIR cardinality and value constraints, instead of silently dropping them. Once the generated modules are regenerated, models obtained via `get_fhir_type()` will enforce constraints that were previously silent — payloads that used to validate (e.g. an empty list for a `1..*` element) will begin raising `ValidationError` ([#406](https://github.com/luisfabib/fhircraft/issues/406))

### Fixed

- Fixed the code generator dropping all Pydantic field constraint metadata when serializing factory-built models to Python source, so generated models were weaker than factory-built ones ([#406](https://github.com/luisfabib/fhircraft/issues/406))
- Fixed `FieldInformation` conflating the scalar string-length constraint (`ElementDefinition.maxLength`) with array cardinality (`ElementDefinition.max`) on a single `max_length` attribute; they are now tracked separately as `max_length` and `max_cardinality` ([#406](https://github.com/luisfabib/fhircraft/issues/406))
- Removed the non-deterministic `# Code autogenerated by Fhircraft … on …` header from the code-generator template; it had been hand-stripped from every checked-in generated file ([#406](https://github.com/luisfabib/fhircraft/issues/406))
